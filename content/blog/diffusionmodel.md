...
$$
$x_0$为建好的高楼大厦（数据样本），$x_T$为拆好的砖瓦水泥（随机噪声），假设“拆楼”需要T步，整个过程可以表示为建高楼大厦的难度在于，从原材料$x_T$到最终高楼大厦$x_0$的跨度过大，普通人很难理解$x_T$是怎么一下子变成$x_0$的。但是，当我们有了“拆楼”的中间过程$x_1$,$x_2$,⋯,$x_T$后，我们知道$x{t−1}$→xt代表着拆楼的一步，那么反过来$x_t$→$x{t−1}$不就是建楼的一步？如果我们能学会两者之间的变换关系$x{t−1}=μ(x_t)$，那么从$x_T$出发，反复地执行$x{T−1}=μ(x_T)$、$x_{T−2}=μ(x{T−1})$、...，最终不就能造出高楼大厦x0出来？

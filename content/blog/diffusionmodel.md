...
设x0为建好的高楼大厦（数据样本），xT为拆好的砖瓦水泥（随机噪声），假设“拆楼”需要T步，整个过程可以表示为x=x0→x1→x2→⋯→xT−1→xT=z建高楼大厦的难度在于，从原材料xT到最终高楼大厦x0的跨度过大，普通人很难理解xT是怎么一下子变成x0的。但是，当我们有了“拆楼”的中间过程x1,x2,⋯,xT后，我们知道xt−1→xt代表着拆楼的一步，那么反过来xt→xt−1不就是建楼的一步？如果我们能学会两者之间的变换关系xt−1=μ(xt)，那么从xT出发，反复地执行xT−1=μ(xT)、xT−2=μ(xT−1)、...，最终不就能造出高楼大厦x0出来？
